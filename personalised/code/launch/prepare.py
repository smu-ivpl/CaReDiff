...
from utils.runid import generate_id
import hydra
import torch
...
def get_last_checkpoint(path, version=None):
    # ckpt_name="last.ckpt"
    output_dir = Path(hydra.utils.to_absolute_path(path))
    if version is None:
        last_ckpt_path = output_dir / "checkpoints"
    else:
        last_ckpt_path = output_dir / f"checkpoints/{version}"
